...
A small demo of the architecture behind [TrainVision](https://trainvision.ai):
...
│                   online/offline status, SyncListener → UI        │
...
│   server.rs     POST/GET /events · GET /events/all · GET /items   │
└──────────────────────────────┬────────────────────────────────────┘
                               │ both depend on
                               ▼
┌───────────────────────────────────────────────────────────────────┐
...
- **Connectivity detection.** The runner's online/offline state is set by a UI
  toggle; a real app feeds it from the platform's reachability watcher.
- **Sync.** For simplicity the demo syncs (push + pull) on a fixed 5-second
  interval. This can be improved: push right after a local change instead of
  waiting for the next tick, and on the pull side let the server notify the app
  when new data is available — over a WebSocket, gRPC streaming, or a push
  notification — so the app fetches only when there is actually something new,
  instead of polling.
...
| `backend/` | axum server: server log, `POST/GET /events`, `GET /events/all`, `GET /items` |
